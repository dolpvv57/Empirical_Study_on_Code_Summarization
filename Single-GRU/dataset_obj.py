...
# 没有sbt
class DatasetObject2(Dataset):
  def __init__(self, keycode_file_path, nl_file_path):
    self.keycode_set = load_data(keycode_file_path)
    self.nl_set = load_data(nl_file_path)
    assert len(self.keycode_set) == len(self.nl_set)
  ...
  def __getitem__(self, idx):
    return self.keycode_set[idx], self.nl_set[idx]
