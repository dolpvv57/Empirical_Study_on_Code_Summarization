...
def test(model_state_dict, vcblry_size, test_loader, nl_vcblry, dataset_size, is_main_model, logger):
  ...
  for b_idx, batch in tqdm(enumerate(test_loader)):
    ...
    with torch.no_grad():
      if not is_main_model:
        ...
        batch_idx_seqs = beam_search_sents(test_model, nl_vcblry, batch_size, keycode_enc_opt, dec_hidden, sbt_enc_opt)
  ...
  with open(config.dataset_base_path + 
            'out/preds_{}_{}'.format(config.output_name, datetime.now()), 'w') as fp, \
    open(config.dataset_base_path + 'out/ref_out_{}'.format(datetime.now()), 'w') as fr:
