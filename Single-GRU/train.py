...
def train(vcblry_size, train_loader, valid_loader, nl_vcblry, dataset_size, is_main_model, logger, pretrain_kyc_enc=None):
  ...
  for epoch in range(config.num_epoch):
    ...
    for b_idx, batch in enumerate(train_loader):
      ...
      if b_idx % config.print_every_batch_num == 0:
        ...
        logger.info('[Time taken: {!s}], epoch: {}/{}, batch: {}/{}, average loss:{:.4f}'.format(datetime.now() - start_time, 
            epoch+1, config.num_epoch, b_idx, total_iter, print_loss))
        print_loss = 0
